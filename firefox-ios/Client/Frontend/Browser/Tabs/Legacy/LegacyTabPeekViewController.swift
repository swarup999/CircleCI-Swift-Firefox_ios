...
class LegacyTabPeekViewController: UIViewController, WKNavigationDelegate {
    ...
    lazy var previewActions: [UIPreviewActionItem] = {
        ...
        let urlIsTooLongToSave = self.tabInstance?.urlIsTooLong ?? false
        let isHomeTab = self.tabInstance?.isFxHomeTab ?? false
        if !self.ignoreURL && !urlIsTooLongToSave {
            if !self.isBookmarked && !isHomeTab {
                actionsBuilder.addBookmark { [weak self] previewAction, viewController in
                    guard let self, let tab = self.tabInstance else { return }
            ...
            if actionsBuilder.count < 3 {
                actionsBuilder.addCopyUrl { [weak self] previewAction, viewController in
                    guard let self, let url = self.tabInstance?.canonicalURL else { return }

                    UIPasteboard.general.url = url
                    self.delegate?.tabPeekDidCopyUrl()
                }
            }
        }
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        if let webViewAccessibilityLabel = tabInstance?.webView?.accessibilityLabel {
        ...
        if let screenshot = tabInstance?.screenshot {
            setupWithScreenshot(screenshot)
        } else {
            setupWebView(tabInstance?.webView)
        }
    }
    ...
    func setState(withProfile browserProfile: BrowserProfile,
        ...
        guard let tab = self.tabInstance,
    ...
    private func createDevicePicker(withProfile browserProfile: BrowserProfile,
        ...
        if let url = tabInstance?.url?.absoluteString {
            clientPickerController.shareItem = ShareItem(url: url, title: tabInstance?.title ?? "")
        }
